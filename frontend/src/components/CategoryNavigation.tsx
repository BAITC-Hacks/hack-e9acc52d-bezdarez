...
export function CategoryNavigation({
  ...
  return (
    <nav aria-label={t('sim.categories')} data-tour="categories" className="flex gap-2 overflow-x-auto pb-1 lg:flex-col lg:overflow-visible">
      {CATEGORIES.map((c, i) => {
          ...
          >
            ...
            <span className="min-w-0 flex-1">
              ...
              <span className={`block truncate text-xs ${chosen ? 'text-good-ink' : 'text-muted'}`}>
                {chosen ? localizeProject(chosen, lang).title : t('sim.notSelected')}
              </span>
            </span>
            <span className="text-sm font-bold tabular-nums text-ink-2">{draft[c].allocatedBudget}</span>
            {chosen && <Check aria-label={t('card.selected')} className="size-4 text-good-ink" />}
          </button>
        )
      })}
    </nav>
