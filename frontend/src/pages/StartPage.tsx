    ...
    <main id="main" className="mx-auto max-w-[1500px] space-y-6 px-3 py-5 sm:px-6">
      <section className="hero rise px-6 py-12 sm:px-12 sm:py-16">
        ...
        <div className="relative mx-auto max-w-3xl text-center">
          <span className="pill border border-white/30 bg-white/15 text-white backdrop-blur">{t('start.chip')}</span>
          <h1 className="mt-5 text-4xl font-black uppercase leading-none tracking-tight sm:text-7xl">{t('start.title')}</h1>
          ...
          <div className="mt-7 flex flex-wrap items-center justify-center gap-3">
            <button
              onClick={onStart}
              className="inline-flex items-center gap-2 rounded-2xl bg-white px-6 py-3.5 text-base font-bold text-[#302a36] shadow-lg transition hover:-translate-y-0.5"
            >
              {hasDraft ? t('nav.continue') : t('nav.start')} <ArrowRight aria-hidden className="size-5 text-accent" />
            </button>
            <span className="text-sm text-white/75">{t('start.noReg')}</span>
          </div>
        </div>
      </section>
      ...
      <div className="grid gap-6 lg:grid-cols-[1.25fr_1fr]">
        <Panel className="rise">
          <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
            <h2 className="text-xl font-bold">{t('start.initialState')}</h2>
            <span className="pill bg-surface-2 text-ink-2">{t('start.beforeDecisions')}</span>
          </div>
          <ScoreGauge value={calculateAqls(BASELINE)} label={t('start.startAqls')} />
        ...
        <div className="space-y-6">
          ...
          <Panel className="rise">
            ...
            <button onClick={onPenalties} className="mt-4 text-sm font-bold text-good-ink hover:underline">
