...
| LLM | NVIDIA API Catalog (`integrate.api.nvidia.com`, по умолчанию `meta/llama-3.3-70b-instruct`) или своя модель на GPU-инстансе NVIDIA Brev (vLLM / NIM) |
...
frontend/src/
  ...
  data/             baseline.ts (исходные показатели, веса), projects.ts (каталог 15 проектов),
  ...
  lib/              calculateSimulation, calculateProjectEffect, calculateSynergies,
                    ...
                    buildAiPayload, explainApi, storage, simulation.test.ts
...
**6. Профиль стратегии** — алгоритм, не LLM: транспорт + сервисы ≥ 55% бюджета → «Технократ»; разброс бюджетов ≤ 10 ед. → «Сбалансированный управленец»; иначе — по сфере с наибольшим бюджетом.
...
|---|---|---|
| `LLM_API_KEY` | Ключ NVIDIA API Catalog (`nvapi-…`, build.nvidia.com). Пусто → шаблонные объяснения | — |
| `LLM_API_URL` | OpenAI-совместимый endpoint. Для своего инстанса на Brev: `https://<host>/v1` | `https://integrate.api.nvidia.com/v1` |
...
```bash
cd frontend && npm test            # 20 тестов движка: формулы, синергии, штрафы, валидация, детерминизм
...
React, Vite, Tailwind CSS, Recharts, Zod, Lucide (MIT/ISC); FastAPI, Pydantic, Uvicorn, python-dotenv (MIT/BSD). LLM: модели NVIDIA API Catalog по их лицензиям. Каталог проектов, коэффициенты и тексты — синтетические, подготовлены командой по ТЗ.
